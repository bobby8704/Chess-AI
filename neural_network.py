...
# Move encoding, legal-move indices and the ONNX session live in inference.py, which
# deliberately does not import torch so the web server can run without it. They are
# re-exported here because callers across the project import them from this module.
from inference import (  # noqa: F401  (re-exported for backwards compatibility)
    POLICY_OUTPUT_SIZE,
    encode_move,
    decode_move,
    legal_move_indices,
